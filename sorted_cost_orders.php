<!-- Displaying orders sorted by cost-->
<?php 
...
<head>
    <meta charset="UTF-8">
    <meta name = "Description" content ="Display Sorted Cost Orders"/>
    <meta name = "keyword" content="Sorted Cost Orders"/>
    <meta name = "author" content ="Vu Thien Tri Phan"/>
    <title>Orders Sorted By Total Cost</title>
...
<body>
    <div id="pages" class="all_but_footer">
        ...
        ?>
        <section>
            <h1 id="heading">Display Orders Sorted By Total Cost</h1>
    <?php
    session_start();
    if (!isset($_POST["sorted_cost"]) && ($_SESSION["message"] == "") && !$_SESSION["cancel"]) { /* If the user hasnt clicked on the "search" button in the "Display Orders Sorted By Total Cost" form AND there isnt any message returned from the update_status.php page AND the user hasnt clicked on the "Cancel" button, then the user can not go to this page! */
        header("location: manager.php");
    } else {
        if (isset($_POST["sorted_cost"])) { /*   If the user clicks on the "search" button on manager page, the message and order id stored in the session by update_status.php page will be reseted*/
	...
	if (!$conn) {
	    ...
	    $orders_table="orders";
		$query = "select order_id, fname, lname, room_type, extra_options, optional_features, partysize, days_travelling, order_time, order_cost, order_status from $orders_table ORDER BY order_cost";
		...
		if(!$result) {
			...
			echo "<table id='query'>\n";
                ...
                while ($record = mysqli_fetch_assoc($result)) {
                    ...
                    echo "<td>";
                    //These 2 below forms I have already described and explained in all_orders.php page
                    echo "<form method='post' action='update_status.php'>
                    <input type='hidden' name= 'order_id' value=",$record["order_id"],">
                    <input type='hidden' name= 'page' value='orders_by_cost'>
                    <label>New Status:
