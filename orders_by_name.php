<!-- This page is used for displaying orders for a customer based on name -->
<?php 
...
<head>
    <meta charset="UTF-8">
    <meta name = "Description" content ="Display Orders By Name"/>
    <meta name = "keyword" content="Orders By Name"/>
    <meta name = "author" content ="Vu Thien Tri Phan"/>
    <title>Orders For A Customer By Name</title>
...
<body>
    <div id="pages" class="all_but_footer">
        ...
        ?>
        <section>
            <h1 id="heading">Display Orders For A Customer By Name</h1>
    <?php
    session_start();
    if (!isset($_POST["search_by_name"])  && ($_SESSION["message"] == "") && !$_SESSION["cancel"]) { /* If the user hasnt clicked on the "search" button in the "Display Orders For A Customer Based On Name" form AND there isnt any message returned from the update_status.php page AND the user hasnt clicked on the "Cancel Button", the user can not go to this page! */
        ...
        if (isset($_POST["search_by_name"])) {
            ...
            $_SESSION["search_by_name"] = $_POST["search_by_name"]; //Store the name from the user's input into a session variable
        }
	...
	if (!$conn) {
        ...
        $name = $_SESSION["search_by_name"]; //Get the name from the session storage
        $name = sanitise_input($name);
        $fullname = explode(" ",$name); // Convert the name from user's input into an array
	    $orders_table="orders";
        if (isset($fullname[0]) && isset($fullname[1])) { //If the user has fully entered first name and last name into the form, then the data will be checked
            $query = "select * from $orders_table where fname='$fullname[0]' and lname='$fullname[1]'"; //Check if is there any record that has first name and last name identical to that from the user's input
		...
		if(!$result) {
			...
			echo "<table id='query'>\n";
                ...
                while ($record = mysqli_fetch_assoc($result)) {
                    ...
                    <input type='hidden' name= 'page' value='orders_by_name'>
                    <label>New Status:
        ...
        echo "<h2>Please fully enter first name and last name</h2>";
    }
        }
